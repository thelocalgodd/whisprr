...
const createGroup = async (req, res) => {
  try {
    ...
    const { name, description, category, type = 'public', maxMembers = 100, requiresApproval = false } = req.body;

    if (req.user.role === 'user' && type === 'support-circle') {
      return res.status(403).json({ error: 'Only counselors and admins can create support circles' });
    }
...
const getGroup = async (req, res) => {
  try {
    ...
    const group = await Group.findById(groupId)
      .populate('creator', 'username profile role counselorInfo.isVerified')
      .populate('moderators', 'username profile role counselorInfo.isVerified')
      .populate('members.user', 'username profile role counselorInfo.isVerified status.isOnline status.lastSeen');
    ...
    res.json({
      ...
      membership: {
        isMember,
        role: memberRole,
        canPost: isMember && (!group.settings.postingPermissions || 
                              group.settings.postingPermissions === 'all' ||
                              (group.settings.postingPermissions === 'counselors-only' && req.user.role === 'counselor') ||
                              (group.settings.postingPermissions === 'moderators-only' && ['moderator', 'admin'].includes(memberRole)))
      }
    });
  } catch (error) {
...
const joinGroup = async (req, res) => {
  try {
    ...
    if (!group.settings.allowedUserTypes.users && req.user.role === 'user') {
      return res.status(403).json({ error: 'This group is restricted to counselors only' });
    }

    if (!group.settings.allowedUserTypes.counselors && req.user.role === 'counselor') {
      return res.status(403).json({ error: 'This group is restricted to users only' });
    }
...
const updateGroup = async (req, res) => {
  try {
    ...
    if (!['admin', 'moderator'].includes(memberRole) && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Insufficient permissions' });
    }

    const allowedUpdates = [
      'name', 'description', 'category', 'tags', 'avatar', 'coverImage',
      'settings.maxMembers', 'settings.requiresApproval', 'settings.allowedUserTypes',
      'settings.autoDelete', 'settings.postingPermissions', 'rules'
    ];
...
const manageMember = async (req, res) => {
  try {
    ...
    if (!['admin', 'moderator'].includes(requesterRole) && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Insufficient permissions' });
    }
    ...
    switch (action) {
      ...
      case 'demote':
        ...
        const demoteMemberIndex = group.members.findIndex(m => m.user.toString() === userId);
      ...
      case 'mute':
        const muteMemberIndex = group.members.findIndex(m => m.user.toString() === userId);
        if (muteMemberIndex !== -1) {
          group.members[muteMemberIndex].isMuted = true;
          if (duration) {
            group.members[muteMemberIndex].mutedUntil = new Date(Date.now() + duration * 60 * 1000);
          }
        }
        break;

      case 'unmute':
        const unmuteMemberIndex = group.members.findIndex(m => m.user.toString() === userId);
    ...
    res.json({
      ...
      group: await Group.findById(groupId).populate('members.user', 'username profile role')
    });
  } catch (error) {
...
const getMyGroups = async (req, res) => {
  try {
    ...
    const groupsWithRole = groups.map(group => {
      const memberInfo = group.members.find(m => m.user.toString() === req.user._id.toString());
      return {
        ...group.toObject(),
...
const scheduleSession = async (req, res) => {
  try {
    ...
    if (!['admin', 'moderator'].includes(memberRole) && req.user.role !== 'counselor') {
      return res.status(403).json({ error: 'Only moderators and counselors can schedule sessions' });
    }
...
const createGroupValidation = [
  ...
  body('category')
    .isIn(['anxiety', 'depression', 'relationships', 'trauma', 'addiction', 'grief', 'self-esteem', 'stress', 'other'])
